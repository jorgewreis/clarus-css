...
## Fase 3 — Qualidade, A11y e performance como gate

- **axe-core** sobre os mockups via Playwright (reusa a infra visual existente)
  como gate no CI (`.github/workflows/ci.yml`).
- **Matriz de a11y** por componente em `docs/reference/` (teclado/ARIA/foco/contraste).
- **Budgets** como blocker no CI (metas do rascunho calibradas ao baseline
  medido em 0.6) + relatório de tamanho gzip por release no CHANGELOG.
- Manter Vitest (unit) e Playwright (regressão visual); atualizar baselines.
